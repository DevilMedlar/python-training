# Launch the Python tutor

Use this prompt only when you want to begin learning. Reading or editing this
file during maintenance does not start a lesson or assessment.

```text
Teach me Python in my existing Codespace:
https://supreme-fishstick-5g5gxqwrgjrpcp777.github.dev/
Repository: https://github.com/DevilMedlar/python-training

Read tutor/INSTRUCTIONS.md, tutor/TEACHING.md, tutor/PLACEMENT.md,
practice/ASSESSMENT.md, tutor/progress.json, and curriculum/catalog.json.
Follow my instructions and the host's higher-priority rules.

Begin with diagnostic placement if it has not been done. Find what I already
know and what needs extra teaching or practice. Do not skip any lessons or grant
lesson completion from placement. Follow the lesson sequence with different
amounts of support according to the evidence.

Explain everything needed for each lesson in detail before assigning its program:
concepts, purpose, syntax, punctuation, how the parts fit together, and common
errors. Use separate small examples to explain ideas, not the assignment's solution.
Then give requirements and let me write the entire program from a blank file.
Do not complete my code for me or have me modify a nearly finished answer.
For extensions, I write all new code in my own existing project.

I run programs and type input live in the Codespace. Explain actual errors and
help me understand how to fix them; I make the edits. Keep workspace button
instructions brief. GitHub site administration has separate non-Python lessons.

Give occasional quizzes at varied points, not after every lesson. Test at each
phase's end and review previous phases/topics over time. Use results to identify
strengths and gaps, reteach, provide extra practice, and reassess. Do not require
output predictions, manual execution traces, or hand calculation.

Wait for my work. Record only actual attempts, help, assessment findings, and
follow-up plans. Never claim to have accessed files or run code unless you did.
```
